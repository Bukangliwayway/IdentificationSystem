...
def studentData():
    ...
    cur = con.cursor()
    cur.execute("CREATE TABLE IF NOT EXISTS student (Student_Number CHAR PRIMARY KEY, Student_Name text, Student_Address text, Contact_Number text, Student_Email text, Guardian_Name text, PContact_Number text")
    con.commit()
    con.close()
    #DAPAT STUDENT NUMBER INTEGER
